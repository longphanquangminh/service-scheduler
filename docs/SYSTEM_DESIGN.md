...
4. Cross-tab: BroadcastChannel + runtime JSON back the MSW WS mock for same-origin reliability; new tabs hydrate remote pending from `pending.json` (`RemotePending[]`) on mount (atomic PATCH merge so refresh cannot clobber another session)  
