...
const PRESENCE_COLORS = ["#c084fc", "#f472b6", "#fb923c", "#a3e635", "#22d3ee", "#e879f9"];
  ...
  if (Array.isArray(parsed)) {
    return parsed.filter(
      (item): item is RemotePending =>
        !!item && typeof item === "object" && typeof (item as RemotePending).sessionId === "string",
    );
  }
  if (parsed && typeof parsed === "object") {
    return Object.values(parsed as Record<string, RemotePending>).filter(item => typeof item?.sessionId === "string");
  }
  return [];
}
...
export async function upsertPendingStore(pending: RemotePending) {
  const next = (await patchPendingJson({ upsert: pending as unknown as PendingUpsert })) as RemotePending[];
